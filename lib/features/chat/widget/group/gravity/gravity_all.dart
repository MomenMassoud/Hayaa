...
class GravityAll extends StatefulWidget{
  String id;
  GravityAll(this.id);
  _GravityAll createState()=>_GravityAll();
}

class _GravityAll extends State<GravityAll>{
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot>(
      stream: _firestore.collection('family').doc(widget.id).collection('user').snapshots(),
      builder: (context,snapshot){
        ...
        return StreamBuilder<QuerySnapshot>(
          stream: _firestore.collection('family').doc(widget.id).collection('count2').snapshots(),
          builder: (context,snapshot){
            if (!snapshot.hasData) {
              return Center(
                child: CircularProgressIndicator(
            ...
            final masseges = snapshot.data?.docs;
            for (var massege in masseges!.reversed){
              FamilyRankModel rank=FamilyRankModel(massege.id, massege.get('user'), massege.get('coin'), massege.get('day'), massege.get('month'), massege.get('year'));
                  ...
                  return StreamBuilder<QuerySnapshot>(
                    ...
                    builder: (context,snapshot){
                      ...
                      if(index==0){
                        ...
                        else if(users[2].valueRank>=10000 && users[index].valueRank<1000000){
                          coin.add("${(users[2].valueRank /1000).toString()} K");
                          print(coin);
                        }
                        else{
                          coin.add("${(users[2].valueRank/1000000).toString()} K");
                        }
                        print("");
                        if(users[1].valueRank<1000){
                          coin.add(users[1].valueRank.toString());
                        }
                        else if(users[1].valueRank>=10000 && users[index].valueRank<1000000){
                          coin.add("${(users[1].valueRank /1000).toString()} K");
                          print(coin);
                        }
                        else{
                          coin.add("${(users[1].valueRank/1000000).toString()} K");
                        }
                        print("");
                        if(users[0].valueRank<1000){
                          coin.add(users[0].valueRank.toString());
                        }
                        else if(users[0].valueRank>=10000 && users[index].valueRank<1000000){
                          coin.add("${(users[0].valueRank /1000).toString()} K");
                          print(coin);
                        }
                        else{
                          coin.add("${(users[0].valueRank/1000000).toString()} K");
                        }
                        return Padding(
                          ...
                          child: Row(
                            ...
                            TopCard(
                              ...
                              cardModel: cards[1],coin:coin[1],coinPic: false),
                            Padding(
                              padding: const EdgeInsets.only(bottom: 100),
                              child: TopCard(
                                screenHeight: screenHeight,
                                screenWidth: screenWidth,
                                cardModel: cards[2],coin: coin[2],coinPic: false),
                            ),
                            TopCard(
                              screenHeight: screenHeight,
                              screenWidth: screenWidth,
                              cardModel: cards[0],coin: coin[0],coinPic: false),
                          ],
                          ),
                        );
                      }
                        ...
                        return Padding(
                          ...
                          child: Column(
                            children: [
                              ListTile(
                                title: Text(users[index].name,style: TextStyle(color: Colors.white,fontSize: 20),),
                                leading: Row(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
                                    Text("${index+1}",style: TextStyle(color: Colors.white,fontSize: 14),),
                                    SizedBox(width: 10,),
                                    CircleAvatar(
                                ...
                                subtitle: Text(users[index].bio,style: TextStyle(color: Colors.white),),
                                trailing: Row(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
                                    Text(users[index].valueRank.toString(),style: TextStyle(color: Colors.yellow),),
                                    SizedBox(width: 10,),
                                    CircleAvatar(
